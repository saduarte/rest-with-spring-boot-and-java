...
public class XlsxExporter implements PersonExporter {
    ...
    public Resource exportPeople(List<PersonDTO> people) throws Exception {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            String[] headers = {"ID", "First Name", "Last Name", "Address", "Gender", "Enable"};

            for (int i = 0; i < headers.length; i++){
                Cell cell = headerRow.createCell(i);
                cell.setCellValue(headers[i]);
                cell.setCellStyle(createHeaderCelStyle(workbook));
            }
    ...
    private CellStyle createHeaderCelStyle(Workbook workbook) {
